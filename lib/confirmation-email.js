...
function buildConfirmationEmailContent(event, portalUrl, { manual = false } = {}) {
  ...
  const confirmUrl = `${portalUrl.replace(/\/$/, "")}/confirmer`;
...
<p>Si quelque chose a changé (horaire, musique, plan de soirée, etc.), vous pouvez le mettre à jour directement via votre <a href="${portalUrl}">espace client</a>.</p>
<p><strong>Si tout est encore correct</strong>, confirmez en un clic — votre dossier sera verrouillé après confirmation :</p>
<p><a href="${confirmUrl}" style="display:inline-block;padding:12px 20px;background:#0d6efd;color:#fff;text-decoration:none;border-radius:6px;font-weight:600;">Tout est correct — confirmer</a></p>
