...
function computePartyDjLockedFields(data) {
  ...
  if (hasYn(g.thematic_evening)) locked.push("thematic_evening");
  if (hasText(g.general_notes)) locked.push("general_notes");
  locked.push(...checkboxItemKeys("age_groups", g.age_groups));
  ...
  if (hasYn(s.meal_bouchee)) locked.push("meal_bouchee");
  ...
  if (hasText(vc.name)) locked.push("venue_contact_name");
  if (hasYn(vc.technical_fog_allowed)) locked.push("venue_contact_technical_fog_allowed");
  if (hasText(vc.phone)) locked.push("venue_contact_phone");
  if (hasText(vc.email)) locked.push("venue_contact_email");
...
function computeWeddingDjLockedFields(data) {
  ...
  if (hasText(g.expected_end_time)) locked.push("expected_end_time");
  if (hasText(g.general_notes)) locked.push("general_notes");
  if (hasYn(g.wedding_has_color)) locked.push("wedding_has_color");
  if (hasText(g.wedding_color)) locked.push("wedding_color");
...
const PORTAL_CLIENT_READONLY_FIELDS = ["venue_contact_technical_fog_allowed"];
...
function getPortalLockedFields(data, eventType) {
  return [...new Set([...getDjLockedFields(data, eventType), ...PORTAL_CLIENT_READONLY_FIELDS])];
}
...
const PARTY_SCALAR_LOCKS = {
  ...
  thematic_evening: "general.thematic_evening",
  ...
  meal_bouchee: "schedule.meal_bouchee",
  ...
  venue_contact_name: "venue_contact.name",
  venue_contact_technical_fog_allowed: "venue_contact.technical_fog_allowed",
  venue_contact_phone: "venue_contact.phone",
  venue_contact_email: "venue_contact.email",
...
function mergePartyQuestionnaireWithLocks(existing, incoming, lockedFields) {
  ...
  out.general.age_groups = mergeCheckboxValues(
    "age_groups",
    locked,
    existing.general?.age_groups,
    incoming.general?.age_groups
  );
...
function mergePortalQuestionnaire(existing, incoming, eventType) {
  const locked = getPortalLockedFields(existing, eventType);
