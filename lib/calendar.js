...
const {
  ...
  parseContractAmountValue,
  formatContractAmountTotal
...
function getEventsInRange(db, startIso, endIso) {
  return db
    .prepare(
      `SELECT e.*, c.first_name_1, c.first_name_2, c.last_name, c.phone
       FROM events e
...
function enrichEventsWithThematicSummary(db, events) {
  const summaries = getThematicSummariesForEventIds(
    db,
    (events || []).map((ev) => ev.id)
  );
  return (events || []).map((ev) => ({
    ...ev,
    thematicSummary: summaries[ev.id] || null
  }));
}

function attachThematicSummaries(db, byDate) {
  const allEvents = Object.values(byDate || {}).flat();
  if (!allEvents.length) return byDate;
  ...
  for (const events of Object.values(byDate)) {
    for (const ev of events) {
      ev.thematicSummary = summaries[ev.id] || null;
    }
  }
  return byDate;
}
...
function getCalendarViewData(db, options = {}) {
  ...
  const events = enrichEventsWithThematicSummary(
    db,
    getEventsInRange(db, rangeStart, rangeEnd)
  );
...
function enrichSubcontractorEvent(ev, subcontractorId, agreementSummaries, locationSummaries = {}) {
  ...
  return {
    ...
    contractAmount: agreement?.amount || null,
    agreementPassed,
    agreementSigned,
...
function getSubcontractorEventsByDate(db, subcontractorId, rangeStart, rangeEnd) {
  const rows = db
    .prepare(
      `SELECT e.*, c.first_name_1, c.first_name_2, c.last_name, c.phone, sc.data AS contract_data
       FROM events e
       JOIN clients c ON c.id = e.client_id
       JOIN subcontractor_contracts sc
  ...
  for (const row of rows) {
    ...
    const enriched = enrichSubcontractorEvent(ev, subcontractorId, agreementSummaries, locationSummaries);

    if (!byDate[calendarDate]) byDate[calendarDate] = [];
    byDate[calendarDate].push(enriched);
  }

  attachThematicSummaries(db, byDate);
...
function formatWeekRangeLabel(weekStartIso, weekEndIso) {
  const startParts = parseDateParam(weekStartIso);
  const endParts = parseDateParam(weekEndIso);
  if (startParts.month === endParts.month && startParts.year === endParts.year) {
    return `${startParts.day}–${endParts.day} ${MONTH_NAMES[startParts.month - 1]} ${startParts.year}`;
  }
  return `${startParts.day} ${MONTH_NAMES[startParts.month - 1]} – ${endParts.day} ${MONTH_NAMES[endParts.month - 1]} ${endParts.year}`;
}

function groupEventsByWeek(events, options = {}) {
  const getDate =
    typeof options.getDate === "function"
      ? options.getDate
      : (ev) => ev.event_date || ev.contractStartDate;
  ...
  for (const ev of events || []) {
    ...
    weekMap.get(weekStart).events.push(ev);
  }
...
function groupEventsByMonth(events, options = {}) {
  const getDate =
    typeof options.getDate === "function"
      ? options.getDate
      : (ev) => ev.event_date || ev.contractStartDate;
...
function sumWeekChargedPrice(events) {
  let total = 0;
  let hasAmount = false;

  for (const ev of events || []) {
    const value = parseContractAmountValue(ev.chargedPrice);
    if (value != null) {
      total += value;
      hasAmount = true;
    }
  ...
  return {
    chargedTotal: hasAmount ? total : null,
    chargedTotalLabel: hasAmount ? formatContractAmountTotal(total) : null
  };
}

function groupAgreementsByWeek(agreementsList) {
  const weeks = groupEventsByWeek(agreementsList, {
    getDate: (ev) => ev.contractStartDate || ev.event_date,
    sortDesc: true
  });

  return weeks.map((week) => {
    const totals = sumWeekChargedPrice(week.events);
    return {
      ...week,
      ...totals,
      contractsTotalLabel: totals.chargedTotalLabel
    };
  });
}

function enrichUpcomingWeekGroups(weeks, todayIso) {
  const currentStart = getWeekStartIso(todayIso);
  const nextStart = addDays(currentStart, 7);

  return weeks.map((week) => {
    const isCurrentWeek = todayIso >= week.weekStart && todayIso <= week.weekEnd;
    const isNextWeek = week.weekStart === nextStart;
    let heading = `Semaine du ${week.weekLabel}`;
    if (isCurrentWeek) heading = "Cette semaine";
    else if (isNextWeek) heading = "Semaine prochaine";

    const totals = sumWeekChargedPrice(week.events);
    const headingWithTotal = totals.chargedTotalLabel
      ? `${heading} — ${totals.chargedTotalLabel}`
      : heading;
...
function enrichUpcomingMonthGroups(months, todayIso) {
  const todayParts = parseDateParam(todayIso);
  const currentMonthKey = monthKey(todayParts.year, todayParts.month);
  let nextYear = todayParts.year;
  let nextMonth = todayParts.month + 1;
  if (nextMonth > 12) {
    nextMonth = 1;
    nextYear += 1;
  }
  ...
  return months.map((month) => {
    ...
    const isNextMonth = month.monthKey === nextMonthKey;
    let heading = month.monthLabel;
    if (isCurrentMonth) heading = "Ce mois-ci";
    else if (isNextMonth) heading = "Mois prochain";

    const totals = sumWeekChargedPrice(month.events);
    const headingWithTotal = totals.chargedTotalLabel
      ? `${heading} — ${totals.chargedTotalLabel}`
      : heading;
...
function getSubcontractorAgreementsList(db, subcontractorId) {
  ...
  const locationSummaries = getEventLocationSummaries(db);
  return events
    .map((ev) => enrichSubcontractorEvent(ev, subcontractorId, agreementSummaries, locationSummaries))
    .sort((a, b) => {
...
function getSubcontractorCalendarData(db, subcontractorId, options = {}) {
  ...
  if (view === "ententes") {
    ...
    return {
      ...
      agreementsByWeek: groupAgreementsByWeek(
        getSubcontractorAgreementsList(db, subcontractorId)
      ),
      prevQuery: null,
      nextQuery: null,
      navPrevLabel: "",
...
module.exports = {
  ...
  groupAgreementsByWeek,
  groupEventsByWeek,
  groupEventsByMonth,
  enrichUpcomingWeekGroups,
  enrichUpcomingMonthGroups,
  sumWeekChargedPrice,
  queryString,
  monthKey
};
