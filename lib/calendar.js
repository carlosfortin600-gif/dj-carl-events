...
function groupAgreementsByWeek(agreementsList) {
  ...
  return weeks.map((week) => {
    ...
    for (const ev of week.events) {
      const value = parseContractAmountValue(ev.contractAmount);
      if (value != null) {
        total += value;
        hasAmount = true;
      }
    ...
    return {
      ...week,
      contractsTotal: hasAmount ? total : null,
      contractsTotalLabel: hasAmount ? formatContractAmountTotal(total) : null
    };
  });
}
