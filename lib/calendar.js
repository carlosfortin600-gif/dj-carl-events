...
function enrichSubcontractorEvent(ev, subcontractorId, agreementSummaries) {
  ...
  return {
    ...
    agreementSigned: agreement?.signed || false,
...
function getSubcontractorAgreementsList(db, subcontractorId) {
  ...
  return events.map((ev) => enrichSubcontractorEvent(ev, subcontractorId, agreementSummaries));
}
...
function getSubcontractorCalendarData(db, subcontractorId, options = {}) {
  ...
  const agreementSummaries = getSubcontractorAgreementSummaries(db, subcontractorId);
  const enrich = (ev) => enrichSubcontractorEvent(ev, subcontractorId, agreementSummaries);

  const byDate = enrichByDate(cal.byDate, enrich);
  ...
  if (dayEvents) {
    dayEvents = dayEvents.map((ev) => enrich(ev));
  }
