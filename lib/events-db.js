...
const { applyTrailerDefaultsToEvent } = require("./trailer-defaults");
...
function getDashboardData(db) {
  ...
  const stats = {
    ...
    incomplete: activeEvents.filter((e) => e.status === "a_completer").length,
    ready: events.filter((e) => e.status === "pret").length,
    inPreparation: activeEvents.filter((e) => !e.client_confirmed_at).length,
    done: events.filter((e) => e.status === "termine").length,
    deleted: getDeletedEvents(db).length
  };

  const upcomingByMonth = enrichUpcomingMonthGroups(
    groupEventsByMonth(upcoming),
    today
  );
...
function getEventChecklist(db, event) {
  ...
  if (!event.city?.trim() && !eventCity(event)) {
    items.push({ label: "Ville", tab: "resume" });
  }
  if (!event.start_time) {
...
function enrichEventsWithChecklist(db, events) {
  const summaries = getEventLocationSummaries(db);
  const thematicSummaries = getThematicSummariesForEventIds(
    db,
    events.map((ev) => ev.id)
  );
  return events.map((ev) => {
    const meta = summaries[ev.id] || {};
    return {
      ...ev,
      checklist: getEventChecklist(db, ev),
      questionnaireSent: Boolean(meta.questionnaireSent),
      questionnaireSentAt: meta.questionnaireSentAt || null,
      questionnaireSentDate: meta.questionnaireSentDate || null,
      questionnaireSentTime: meta.questionnaireSentTime || null,
      questionnaireSentValue: meta.questionnaireSentValue || null,
      chargedPrice: meta.chargedPrice || null,
      thematicSummary: thematicSummaries[ev.id] || null,
      questionnaireSentLogs: meta.questionnaireSentLogs || []
    };
  });
}
...
function createEvent(db, payload) {
  ...
  const insertEvent = db.prepare(`
    INSERT INTO events (
      ...
      venue, venue_elevator, address, city, guest_count, status
    ) VALUES (
      @client_id, @event_type, @event_date, @start_time, @end_date, @end_time,
      @venue, @venue_elevator, @address, @city, @guest_count, 'a_completer'
    )
  `);
