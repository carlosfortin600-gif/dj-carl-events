...
function saveDjNotes(db, eventId, data) {
  ...
  if (saveDepart) {
    ...
    techEmailRdvSent = data.tech_email_rdv_sent === "yes" ? "yes" : "no";
    techEmailRdvDate = null;
    techEmailRdvTime = null;
    if (isYes(techEmailRdvSent)) {
      if (data.tech_email_rdv_datetime !== undefined) {
        const parsed = parseStoredDatetime(data, "tech_email_rdv_datetime");
        techEmailRdvDate = parsed.date;
        techEmailRdvTime = parsed.time;
      } else {
        techEmailRdvDate = data.tech_email_rdv_date?.trim() || null;
        techEmailRdvTime = data.tech_email_rdv_time?.trim() || null;
      }
    }
...
function summarizeLocationNotes(notes, questionnaireSentLogs = []) {
  const latestLog = questionnaireSentLogs[0] || null;
  const questionnaireSentAt = latestLog
    ? formatSentAt(latestLog.sent_date, latestLog.sent_time)
  ...
  return {
    ...
    emailRdvSent: isYes(notes.tech_email_rdv_sent),
    emailRdvAt: formatStoredDatetime(notes.tech_email_rdv_date, notes.tech_email_rdv_time),
    ...
    questionnaireSentAt,
    questionnaireSentDate: latestLog?.sent_date || notes.tech_questionnaire_sent_date || null,
    questionnaireSentTime: latestLog?.sent_time || notes.tech_questionnaire_sent_time || null,
...
function getEventLocationSummaries(db) {
  ...
  for (const row of rows) {
    summaries[row.event_id] = summarizeLocationNotes(row, logsGrouped[row.event_id] || []);
  }
  return summaries;
}
