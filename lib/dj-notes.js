    ...
    db
      .prepare(
        `SELECT content, updated_at,
                ...
                tech_room_needed, tech_room_location, tech_room_price,
...
function saveDjNotes(db, eventId, data) {
  ...
  let techRoomPrice = existing.tech_room_price;
  ...
  if (saveLocation) {
    ...
    if (isYes(techRoomNeeded)) {
      ...
      techRoomPrice = data.tech_room_price?.trim() || null;
...
function getEventHotelSummaries(db) {
  const rows = db
    .prepare(
      `SELECT event_id, tech_room_needed, tech_room_price
       FROM dj_notes`
    )
    .all();
  ...
  for (const row of rows) {
    summaries[row.event_id] = {
      rented: row.tech_room_needed === "yes",
      price: row.tech_room_price?.trim() || null
    };
  }
  return summaries;
}
