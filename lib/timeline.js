...
function buildStepsFromWeddingQuestionnaire(q) {
  ...
  if (q.general?.expected_end_time) {
    addStep(q.general.expected_end_time, "Fin de la danse / fin de soirée", null);
  }
...
function buildStepsFromQuestionnaire(q, eventType) {
  ...
  if (q._formType === "party" || (eventType && !isWeddingEvent(eventType))) {
    steps = buildStepsFromPartyQuestionnaire(q);
  } else {
    steps = buildStepsFromWeddingQuestionnaire(q);
  }

  return [...steps].sort((a, b) => compareTimelineTime(a.time, b.time));
}
