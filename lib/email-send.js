...
function describeMailError(err) {
  ...
  if (code === "ERESEND" || lower.includes("resend") || lower.includes("domain")) {
    if (lower.includes("domain") || lower.includes("from") || lower.includes("verify")) {
      return `${response} — expéditeur Resend invalide. Sans domaine vérifié, utilisez exactement : ${RESEND_TEST_FROM}`;
    }
    if (lower.includes("only send") || lower.includes("testing")) {
      return `${response} — en mode test Resend, le courriel de notification doit être le même que votre compte Resend.`;
    }
    return response;
  }
