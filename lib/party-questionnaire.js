const {
  parseCheckboxList,
  yn,
  getQuestionnaireRow,
  saveQuestionnaireRow,
  isWeddingEvent,
  PARTY_DECADES,
  ANIMATION_LEVELS,
  ANIMATION_ACTIVITIES,
  GUEST_REQUEST_OPTIONS,
  isOtherCocktailLocation,
  normalizeCocktailLocationFields
} = require("./questionnaire-shared");
const { parseClientPlanSteps, defaultClientPlan, mergeClientPlan } = require("./plan-soiree");
const { normalizePhone } = require("./phone");
const {
  parseSpecialAnimation,
  defaultSpecialAnimation,
  mergeSpecialAnimation
} = require("./special-animation");

const AGE_GROUPS = [
  "18-30 ans",
  "31-45 ans",
  "46-60 ans",
  "60 ans et +",
  "Tous âges"
];

const EVENING_THEME_OPTIONS = [
  "Country",
  "Croisière",
  "Chic / Oscar",
  "Gatsby",
  "Autre"
];

function normalizeEveningThemeLabel(theme) {
  if (theme === "Corpo") return "Gatsby";
  return theme;
}

const PARTY_MOMENT_KEYS = [
  { key: "welcome", label: "Accueil / cocktail" },
  { key: "dinner", label: "Repas / souper" },
  { key: "special_1", label: "Moment spécial 1" },
  { key: "special_2", label: "Moment spécial 2" },
  { key: "special_3", label: "Moment spécial 3" },
  { key: "dance_floor", label: "Ouverture du plancher" },
  { key: "last_song", label: "Dernière chanson" }
];

const PARTY_GUEST_REQUEST_OPTIONS = GUEST_REQUEST_OPTIONS.map((opt) =>
  opt.value === "ask_couple"
    ? { value: "ask_client", label: "Demander au client" }
    : opt
);

function defaultPartyQuestionnaire() {
  return {
    _formType: "party",
    _completed: false,
    general: {
      guest_arrival_time: "",
      party_start_time: "",
      expected_end_time: "",
      celebration_notes: "",
      age_range: "",
      age_groups: [],
      thematic_evening: "",
      evening_themes: [],
      evening_theme_other: "",
      general_notes: ""
    },
    schedule: {
      has_cocktail: "",
      cocktail_location: "",
      cocktail_place: "",
      cocktail_dj_distance: "",
      cocktail_equipment_provided: "",
      has_meal: "",
      meal_service_type: "",
      meal_bouchee: "",
      speeches_planned: "",
      special_moments_notes: ""
    },
    cocktail: {
      styles: [],
      requests: ""
    },
    dinner: {
      styles: [],
      requests: ""
    },
    party_music: {
      decades: [],
      must_play: "",
      preferred_artists: "",
      forbidden_songs: "",
      avoid_styles: "",
      guest_requests: ""
    },
    animation: {
      level: "",
      activities: [],
      comments: ""
    },
    important_moments: PARTY_MOMENT_KEYS.map((m) => ({
      key: m.key,
      label: m.label,
      active: "",
      time: "",
      song: "",
      notes: ""
    })),
    important_questions: {
      perfect_evening: "",
      other_info: "",
      sensitive_situations: ""
    },
    day_contact: {
      name: "",
      phone: "",
      email: ""
    },
    venue_contact: {
      name: "",
      phone: "",
      email: "",
      technical_fog_allowed: ""
    },
    client_plan: defaultClientPlan(),
    special_animation: defaultSpecialAnimation()
  };
}

function bodyToPartyQuestionnaire(body) {
  const data = defaultPartyQuestionnaire();
  data._completed = body._completed === "1";

  Object.assign(data.general, {
    guest_arrival_time: body.guest_arrival_time || "",
    party_start_time: body.party_start_time || "",
    expected_end_time: body.expected_end_time || "",
    celebration_notes: body.celebration_notes || "",
    age_range: body.age_range || "",
    thematic_evening: yn(body.thematic_evening),
    general_notes: body.general_notes || ""
  });
  data.general.age_groups = parseCheckboxList(body, "age_groups");
  if (data.general.thematic_evening === "no") {
    data.general.evening_themes = [];
    data.general.evening_theme_other = "";
  } else {
    data.general.evening_themes = parseCheckboxList(body, "evening_themes").map(
      normalizeEveningThemeLabel
    );
    data.general.evening_theme_other = body.evening_theme_other || "";
    if (data.general.evening_theme_other && !data.general.evening_themes.includes("Autre")) {
      data.general.evening_themes.push("Autre");
    }
  }

  Object.assign(data.schedule, {
    has_cocktail: yn(body.has_cocktail),
    cocktail_location: body.cocktail_location || "",
    cocktail_place: body.cocktail_place || "",
    cocktail_dj_distance: body.cocktail_dj_distance || "",
    cocktail_equipment_provided: yn(body.cocktail_equipment_provided),
    has_meal: yn(body.has_meal),
    meal_service_type: body.meal_service_type || "",
    meal_bouchee: yn(body.meal_bouchee),
    speeches_planned: yn(body.speeches_planned),
    special_moments_notes: body.special_moments_notes || ""
  });

  if (data.schedule.has_cocktail === "no") {
    data.schedule.cocktail_location = "";
    data.schedule.cocktail_place = "";
    data.schedule.cocktail_dj_distance = "";
    data.schedule.cocktail_equipment_provided = "";
  } else {
    if (!isOtherCocktailLocation(data.schedule.cocktail_location)) {
      data.schedule.cocktail_equipment_provided = "";
    }
    const cocktailFields = normalizeCocktailLocationFields(
      data.schedule.cocktail_location,
      data.schedule.cocktail_equipment_provided,
      data.schedule.cocktail_place,
      data.schedule.cocktail_dj_distance
    );
    data.schedule.cocktail_place = cocktailFields.place;
    data.schedule.cocktail_dj_distance = cocktailFields.distance;
  }

  data.cocktail.styles = parseCheckboxList(body, "cocktail_styles");
  data.cocktail.requests = body.cocktail_requests?.trim() || "";
  data.dinner.styles = parseCheckboxList(body, "dinner_styles");
  data.dinner.requests = body.dinner_requests?.trim() || "";

  data.party_music.decades = parseCheckboxList(body, "party_decades");
  data.party_music.must_play = body.must_play || "";
  data.party_music.preferred_artists = body.preferred_artists || "";
  data.party_music.forbidden_songs = body.forbidden_songs || "";
  data.party_music.avoid_styles = body.avoid_styles || "";
  data.party_music.guest_requests = body.guest_requests || "";

  data.animation.level = body.animation_level || "";
  data.animation.activities = parseCheckboxList(body, "animation_activities");
  data.animation.comments = body.animation_comments || "";

  data.important_moments = PARTY_MOMENT_KEYS.map((m) => ({
    key: m.key,
    label: m.label,
    active: yn(body[`moment_${m.key}_active`]),
    time: body[`moment_${m.key}_time`] || "",
    song: body[`moment_${m.key}_song`] || "",
    notes: body[`moment_${m.key}_notes`] || ""
  }));

  Object.assign(data.important_questions, {
    perfect_evening: body.perfect_evening || "",
    other_info: body.other_info || "",
    sensitive_situations: body.sensitive_situations || ""
  });

  Object.assign(data.day_contact, {
    name: body.day_contact_name || "",
    phone: normalizePhone(body.day_contact_phone) || "",
    email: body.day_contact_email || ""
  });

  Object.assign(data.venue_contact, {
    name: body.venue_contact_name || "",
    phone: normalizePhone(body.venue_contact_phone) || "",
    email: body.venue_contact_email || "",
    technical_fog_allowed: yn(body.venue_contact_technical_fog_allowed)
  });

  data.client_plan = { steps: parseClientPlanSteps(body) };
  data.special_animation = parseSpecialAnimation(body);

  return data;
}

function mergePartyQuestionnaire(stored) {
  const base = defaultPartyQuestionnaire();
  if (!stored || typeof stored !== "object") return base;

  const merged = { ...base, ...stored };
  merged.general = { ...base.general, ...(stored.general || {}) };
  if (!Array.isArray(merged.general.age_groups)) {
    merged.general.age_groups = [];
  }
  if (!Array.isArray(merged.general.evening_themes)) {
    merged.general.evening_themes = [];
  } else {
    merged.general.evening_themes = merged.general.evening_themes.map(normalizeEveningThemeLabel);
  }
  if (typeof merged.general.evening_theme_other !== "string") {
    merged.general.evening_theme_other = "";
  }
  if (!merged.general.thematic_evening) {
    if (merged.general.evening_themes.length || merged.general.evening_theme_other) {
      merged.general.thematic_evening = "yes";
    }
  }
  if (!merged.general.age_groups.length && merged.general.age_range) {
    merged.general.age_groups = merged.general.age_range
      .split(",")
      .map((part) => part.trim())
      .filter((part) => AGE_GROUPS.includes(part));
  }
  if (!merged.general.age_range && merged.general.age_groups.length) {
    merged.general.age_range = merged.general.age_groups.join(", ");
  } else if (!merged.general.age_range && Array.isArray(stored.general?.age_groups) && stored.general.age_groups.length) {
    merged.general.age_range = stored.general.age_groups.join(", ");
  }
  if (typeof merged.general.general_notes !== "string") {
    merged.general.general_notes = "";
  }
  merged.schedule = { ...base.schedule, ...(stored.schedule || {}) };
  merged.cocktail = { ...base.cocktail, ...(stored.cocktail || {}) };
  merged.dinner = { ...base.dinner, ...(stored.dinner || {}) };
  merged.party_music = { ...base.party_music, ...(stored.party_music || {}) };
  merged.animation = { ...base.animation, ...(stored.animation || {}) };
  merged.important_questions = {
    ...base.important_questions,
    ...(stored.important_questions || {})
  };
  merged.day_contact = { ...base.day_contact, ...(stored.day_contact || {}) };
  merged.venue_contact = { ...base.venue_contact, ...(stored.venue_contact || {}) };
  merged.client_plan = mergeClientPlan(stored.client_plan);
  merged.special_animation = mergeSpecialAnimation(stored.special_animation);

  if (Array.isArray(stored.important_moments)) {
    merged.important_moments = base.important_moments.map((item) => {
      const found = stored.important_moments.find((m) => m.key === item.key);
      return found ? { ...item, ...found } : item;
    });
  }

  if (Array.isArray(stored.party_music?.must_play)) {
    merged.party_music.must_play = stored.party_music.must_play
      .filter((song) => song.artist || song.title)
      .map((song) => {
        const artist = (song.artist || "").trim();
        const title = (song.title || "").trim();
        if (artist && title) return `${artist} — ${title}`;
        return artist || title;
      })
      .join("\n");
  } else if (typeof stored.party_music?.must_play !== "string") {
    merged.party_music.must_play = "";
  }

  return merged;
}

function formatThematicEveningSummary(general) {
  if (!general || typeof general !== "object") return null;

  const thematic = general.thematic_evening;
  if (thematic === "no") return "Pas de thématique";

  const themes = [...(general.evening_themes || [])].filter(Boolean);
  const hasThemes = themes.length > 0 || Boolean(general.evening_theme_other?.trim());

  if (thematic === "yes" || hasThemes) {
    const labels = themes
      .map(normalizeEveningThemeLabel)
      .filter((theme) => theme !== "Autre");
    if (themes.includes("Autre") && general.evening_theme_other?.trim()) {
      labels.push(general.evening_theme_other.trim());
    } else if (themes.includes("Autre") && !labels.length) {
      labels.push("Autre");
    }
    if (labels.length) {
      return `Thématique ${labels.join(", ")}`;
    }
    return "Thématique oui";
  }

  return null;
}

function getThematicSummariesForEventIds(db, eventIds) {
  const map = {};
  const ids = [...new Set((eventIds || []).filter(Boolean))];
  if (!ids.length) return map;

  const placeholders = ids.map(() => "?").join(", ");
  const rows = db
    .prepare(
      `SELECT event_id, data FROM wedding_questionnaires WHERE event_id IN (${placeholders})`
    )
    .all(...ids);

  for (const row of rows) {
    try {
      const parsed = JSON.parse(row.data || "{}");
      const summary = formatThematicEveningSummary(parsed.general);
      if (summary) map[row.event_id] = summary;
    } catch {
      // ignore invalid questionnaire JSON
    }
  }

  return map;
}

function getPartyQuestionnaire(db, eventId) {
  const row = getQuestionnaireRow(db, eventId);
  if (!row) return { data: defaultPartyQuestionnaire(), updated_at: null };

  try {
    return {
      data: mergePartyQuestionnaire(JSON.parse(row.data || "{}")),
      updated_at: row.updated_at
    };
  } catch {
    return { data: defaultPartyQuestionnaire(), updated_at: row.updated_at };
  }
}

function savePartyQuestionnaire(db, eventId, data) {
  saveQuestionnaireRow(db, eventId, data);
}

function isPartyQuestionnaireComplete(data) {
  if (data._completed) return true;

  const hasSchedule =
    Object.values(data.schedule || {}).some(Boolean) ||
    Object.values(data.general || {}).some((v) =>
      Array.isArray(v) ? v.length > 0 : Boolean(v)
    );
  const hasMusic =
    (data.party_music?.decades || []).length > 0 ||
    Boolean(data.party_music?.must_play?.trim());
  const hasText = Object.values(data.important_questions || {}).some(
    (v) => v && String(v).trim().length > 15
  );

  return hasSchedule && (hasMusic || hasText);
}

module.exports = {
  AGE_GROUPS,
  EVENING_THEME_OPTIONS,
  PARTY_MOMENT_KEYS,
  PARTY_GUEST_REQUEST_OPTIONS,
  defaultPartyQuestionnaire,
  bodyToPartyQuestionnaire,
  getPartyQuestionnaire,
  savePartyQuestionnaire,
  isPartyQuestionnaireComplete,
  mergePartyQuestionnaire,
  formatThematicEveningSummary,
  getThematicSummariesForEventIds
};
