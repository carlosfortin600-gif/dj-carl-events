...
function eventCity(event) {
  ...
  const looksLikeProvincePostal =
    /^(QC|Québec|Quebec|PQ|ON|Ontario|NB|NS|PE|NL|MB|SK|AB|BC|YT|NT|NU)(\s|$)/i.test(last) ||
    /\b[A-Z]\d[A-Z]\s?\d[A-Z]\d\b/i.test(last);

  if (looksLikeProvincePostal) {
