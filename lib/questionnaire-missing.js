...
function shouldSkipField(name, q, wedding) {
  ...
  if (!wedding && ["cocktail_location", "cocktail_place", "cocktail_dj_distance", "cocktail_equipment_provided"].includes(name)) {
    if (q.schedule?.has_cocktail === "no") return true;
  }

  const ceremonyRoom = q.ceremony?.ceremony_room || q.ceremony?.location || "";
  const isOtherLocation = (value) => value === "other_room" || value === "outside";

  if (name === "ceremony_place" && !isOtherLocation(ceremonyRoom)) return true;
  if (name === "ceremony_equipment_provided" && !isOtherLocation(ceremonyRoom)) return true;
  if (name === "ceremony_sound_provided" && ceremonyRoom !== "same_room") return true;

  const weddingCocktailLoc = q.cocktail?.cocktail_location || "";
  const partyCocktailLoc = q.schedule?.cocktail_location || "";
  const cocktailLoc = wedding ? weddingCocktailLoc : partyCocktailLoc;
  const cocktailKit = wedding ? q.cocktail?.equipment_provided : q.schedule?.cocktail_equipment_provided;

  if (name === "cocktail_place") {
    if (!wedding && q.schedule?.has_cocktail === "no") return true;
    if (!isOtherLocation(cocktailLoc)) return true;
  }

  if (name === "cocktail_dj_distance") {
    if (!wedding && q.schedule?.has_cocktail === "no") return true;
    if (!isOtherLocation(cocktailLoc)) return true;
    if (cocktailKit !== "yes") return true;
  }

  if (name === "cocktail_equipment_provided" && wedding && !isOtherLocation(weddingCocktailLoc)) return true;

  if (!wedding && name === "cocktail_equipment_provided") {
    if (!isOtherLocation(partyCocktailLoc)) return true;
  }

  if (name === "evening_theme_other") {
    if (q.general?.thematic_evening === "no") return true;
    const themes = q.general?.evening_themes || [];
    if (!themes.includes("Autre")) return true;
  }

  if (!wedding && (name === "evening_themes" || name.startsWith("evening_themes:"))) {
    if (q.general?.thematic_evening === "no") return true;
  }
...
function getWeddingMissingFields(q) {
  ...
  checkText(items, q, wedding, "Responsable de la salle", "Nom", "venue_contact_name", vc.name);
  checkRadio(items, q, wedding, "Responsable de la salle", "Brouillard technique autorisé", "venue_contact_technical_fog_allowed", vc.technical_fog_allowed);
  checkText(items, q, wedding, "Responsable de la salle", "Téléphone", "venue_contact_phone", vc.phone);
  checkText(items, q, wedding, "Responsable de la salle", "Courriel", "venue_contact_email", vc.email);

  checkRadio(items, q, wedding, "Cérémonie", "Lieu", "ceremony_room", ceremony.ceremony_room);
  checkText(items, q, wedding, "Cérémonie", "Endroit", "ceremony_place", ceremony.ceremony_place || ceremony.location);
  checkRadio(items, q, wedding, "Cérémonie", "Équipement selon l'entente", "ceremony_equipment_provided", ceremony.equipment_provided);
  checkRadio(items, q, wedding, "Cérémonie", "Sono selon l'entente", "ceremony_sound_provided", ceremony.sound_provided);
  ...
  checkRadio(items, q, wedding, "Cocktail", "Deuxième kit de son", "cocktail_equipment_provided", cd.equipment_provided);
  ...
  checkCheckboxGroup(items, q, wedding, "Repas", "Style(s) de musique", "dinner_styles", dn.styles);
  checkText(items, q, wedding, "Repas", "Commentaires", "dinner_requests", dn.requests);
...
function getPartyMissingFields(q) {
  ...
  checkText(items, q, wedding, "Informations générales", "Tranche d'âge des invités", "age_range", g.age_range);
  checkRadio(items, q, wedding, "Informations générales", "Soirée thématique", "thematic_evening", g.thematic_evening);
  checkCheckboxGroup(items, q, wedding, "Informations générales", "Thème", "evening_themes", g.evening_themes);
  ...
  checkText(items, q, wedding, "Déroulement", "Type de souper", "meal_service_type", sch.meal_service_type);
  checkRadio(items, q, wedding, "Déroulement", "Bouchée", "meal_bouchee", sch.meal_bouchee);
  ...
  checkRadio(items, q, wedding, "Déroulement", "Deuxième kit de son", "cocktail_equipment_provided", sch.cocktail_equipment_provided);
  ...
  checkCheckboxGroup(items, q, wedding, "Musique cocktail", "Styles désirés", "cocktail_styles", cd.styles);
  checkText(items, q, wedding, "Musique cocktail", "Demandes spéciales", "cocktail_requests", cd.requests);
  checkCheckboxGroup(items, q, wedding, "Musique souper", "Style(s) de musique", "dinner_styles", dn.styles);
  checkText(items, q, wedding, "Musique souper", "Commentaires", "dinner_requests", dn.requests);
