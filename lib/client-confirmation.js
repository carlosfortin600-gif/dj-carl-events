    ...
    .prepare(
      `SELECT e.*,
       ...
       WHERE e.portal_token = ?
         ...
         AND e.confirmation_email_sent_at IS NOT NULL`
    )
    .get(token);
}
...
function getPortalAccessDeniedReason(db, token) {
  const row = db
    .prepare(
      `SELECT portal_enabled, client_confirmed_at, deleted_at, confirmation_email_sent_at
  ...
  if (!row.confirmation_email_sent_at) return "confirm_not_requested";
  return null;
}
...
function portalAccessDeniedMessage(reason) {
  ...
  if (reason === "confirm_not_requested") {
    return "La confirmation n'est pas encore disponible. DJ Carl vous enverra un courriel avec le lien de confirmation lorsque ce sera le moment.";
  }
