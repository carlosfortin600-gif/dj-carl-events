...
const { getEventServices, getAllEvents } = require("./events-db");
const { clientShortName, formatTime, localTimestampNow, todayLocal } = require("./helpers");
...
function ensureUniqueFolderName(baseName, usedNames, eventId) {
  let name = baseName;
  if (!usedNames.has(name)) {
    usedNames.add(name);
    return name;
  }
  name = `${baseName}-evt${eventId}`;
  if (!usedNames.has(name)) {
    usedNames.add(name);
    return name;
  }
  let n = 2;
  while (usedNames.has(`${baseName}-evt${eventId}-${n}`)) n += 1;
  name = `${baseName}-evt${eventId}-${n}`;
  usedNames.add(name);
  return name;
}

async function buildEventDossierFiles(db, event) {
  ...
  const { getQuestionnaireSentLogs } = require("./questionnaire-sent-log");
  const { getEmailRdvSentLogs } = require("./email-rdv-sent-log");
  const { getClientContactLogs } = require("./client-contact-log");
  ...
  const summarySheet = buildSummarySheet({
    ...
    timelineItems,
    djNotes,
    questionnaireSentLogs: getQuestionnaireSentLogs(db, event.id),
    emailRdvSentLogs: getEmailRdvSentLogs(db, event.id),
    clientContactLogs: getClientContactLogs(db, event.id)
  });
...
async function buildEventDossierZip(db, event) {
  const { folderName, files } = await buildEventDossierFiles(db, event);
  ...
  return {
    folderName,
    zipBuffer: await zipFiles(entries)
  };
}

async function buildAllEventsDossierZip(db, { upcomingOnly = true } = {}) {
  const today = todayLocal();
  let events = getAllEvents(db);
  if (upcomingOnly) {
    events = events.filter((event) => event.event_date >= today);
  }
  ...
  for (const event of events) {
    const { folderName, files } = await buildEventDossierFiles(db, event);
    const uniqueFolder = ensureUniqueFolderName(folderName, usedNames, event.id);
    for (const file of files) {
      entries.push({
        path: `${uniqueFolder}/${file.name}`,
        buffer: file.buffer
      });
    }
  }

  const stamp = (localTimestampNow() || "").slice(0, 10) || "export";
  return {
    archiveName: `dj-carl-dossiers-a-venir-${stamp}.zip`,
...
module.exports = {
  buildEventDossierFiles,
  buildEventDossierZip,
  buildAllEventsDossierZip,
  sanitizeFolderName
};
