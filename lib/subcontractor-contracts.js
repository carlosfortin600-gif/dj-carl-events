...
const { parseEndDatetime, formatDateTimeRangeFr, localTimestampNow, isContractEndBeforeStart } = require("./helpers");

const SUBCONTRACTORS = [
  { id: "eric", label: "Éric Moreault" },
  { id: "mario", label: "Mario" },
  { id: "francis", label: "Francis Martel" }
];
...
const CONTRACT_DEFAULTS = {
  ...
  eric: {
    full_name: "Éric Moreault",
    service_description:
      "Prestation de services techniques pour l'événement (montage, opération et démontage selon les besoins convenus).",
    payment_terms: "Paiement à la fin de la prestation, sauf entente contraire."
  },
  francis: {
    full_name: "Francis Martel",
    service_description:
      "Prestation de services techniques pour l'événement (montage, opération et démontage selon les besoins convenus).",
    payment_terms: "Paiement à la fin de la prestation, sauf entente contraire."
  }
...
function isContractFullySigned(contract) {
  return !!(contract?.signature_contractant && contract?.signature_subcontractor);
}
...
function resolveSignatureDate(previousSignature, nextSignature, previousDate) {
  const next = nextSignature?.trim() || null;
  if (!next) return null;
  const prev = previousSignature?.trim() || null;
  if (!prev || prev !== next) return localTimestampNow();
...
function saveSubcontractorContract(db, eventId, subcontractorId, body) {
  ...
  if (
    ...
    return {
      ok: false,
      error: "La date de fin doit être au minimum la date et l'heure de début."
    };
  }
  ...
  const data = {
    ...
    signature_contractant: nextContractantSignature,
    signature_contractant_date: resolveSignatureDate(
      existing.signature_contractant,
      nextContractantSignature,
      existing.signature_contractant_date
    ),
    signature_subcontractor: nextSubcontractorSignature,
    signature_subcontractor_date: resolveSignatureDate(
      existing.signature_subcontractor,
      nextSubcontractorSignature,
      existing.signature_subcontractor_date
    )
  };
...
function saveSubcontractorSignatureOnly(db, eventId, subcontractorId, signature) {
  ...
  const data = {
    ...existing,
    signature_subcontractor: sig,
    signature_subcontractor_date: sig ? localTimestampNow() : null,
...
function getContractCalendarDate(contract, eventDate) {
  return contract?.contract_date || eventDate || null;
}

function getContractCalendarTime(contract, eventStartTime) {
  return contract?.contract_time || eventStartTime || null;
}

function parseContractAmountValue(amountText) {
  if (!amountText?.trim()) return null;
  const match = amountText.match(/(\d[\d\s]*(?:[.,]\d{1,2})?)/);
...
function formatContractAmountTotal(total) {
...
function getSubcontractorAgreementSummaries(db, subcontractorId) {
  ...
  for (const row of rows) {
    ...
    summaries[row.event_id] = {
      ...
      contractStartDate: contract.contract_date || null,
      contractStartTime: contract.contract_time || null
    };
  }
