...
function buildDjNotesSection(djNotes, emailRdvSentLogs = []) {
  const trailerRow =
  ...
  return section("Notes DJ (privées)", [
    ...
    row(
      "Client contacté",
      djNotes?.tech_client_called === "yes"
        ? formatDateTimeFr(djNotes?.tech_client_call_date, djNotes?.tech_client_call_time) || "Oui"
        : djNotes?.tech_client_called === "no"
          ? "Non"
          : "—"
    ),
...
function buildSummarySheet({
  ...
  const sections = [
    ...
    buildDjNotesSection(djNotes, emailRdvSentLogs),
