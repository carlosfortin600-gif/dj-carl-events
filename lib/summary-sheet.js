...
function buildAnimationSection(q) {
  ...
  const specialItems = (q.special_animation?.items || []).filter(
    (item) => item.label?.trim() || item.song?.trim()
  );
  ...
  specialItems.forEach((item, index) => {
    const label = item.label?.trim() || `Animation ${index + 1}`;
    rows.push(row(label, item.song?.trim() || "—"));
  });
