const { formatDateFr, formatTime, formatDateTimeFr, formatTimestampFr, formatDateRangeFr, formatDateTimeRangeFr, localTimestampNow, endDatetimeLocalValue, statusLabel, clientFullName, googleMapsUrl, googleMapsDirectionsUrl, compareChronologicalTime, eventCity } = require("./helpers");
const { telHref, formatPhoneDisplay } = require("./phone");
...
function phoneRow(label, value) {
  ...
  if (formatted === "—") return row(label, "—");
  return row(label, formatted, telHref(value));
}
...
function buildEventHeaderSection(event) {
  ...
  return section("Client et événement", [
    ...
    phoneRow("Téléphone", event.phone),
    ...
    row("Ascenseur", ynLabel(event.venue_elevator)),
    row("Adresse", event.address, googleMapsUrl(event.address)),
    row("Ville", event.city || eventCity(event)),
...
function buildServicesSection(services, event) {
  ...
  if (event?.animation_notes && items.includes("Animation")) {
    const index = items.indexOf("Animation");
    items[index] = `Animation (${event.animation_notes})`;
  }
...
function buildDayContactSection(q) {
  ...
  return section("Personne à contacter le jour J", [
    row("Nom", dc.name),
    phoneRow("Téléphone", dc.phone),
    row("Courriel", dc.email)
  ]);
}

function buildVenueContactSection(q) {
  const vc = q.venue_contact || {};
  return section("Personne responsable de la salle", [
    row("Nom", vc.name),
    row("Brouillard technique autorisé", ynLabel(vc.technical_fog_allowed)),
    phoneRow("Téléphone", vc.phone),
    row("Courriel", vc.email)
  ]);
}

function buildAnimationSection(q) {
  const anim = q.animation || {};
  const specialNotes = q.special_animation?.notes?.trim();
  ...
  if (specialNotes) {
    rows.push(row("Animation spéciale", specialNotes));
  }

  return section("Animation", rows);
}
...
function buildMomentsSection(q, momentKeys) {
  ...
  const rows = momentKeys
    ...
    .map((entry) => row(entry.label, entry.value));
...
function buildWeddingQuestionnaireSections(q) {
  ...
  const cocktail = q.cocktail || {};
  const dinner = q.dinner || {};
  ...
  return [
    section("Informations générales", [
      ...
      row(
        "Couleur spécifique pour le mariage",
        g.wedding_has_color === "yes"
          ? g.wedding_color || "—"
          : g.wedding_has_color === "no"
            ? "Non"
            : "—"
      )
    ]),
    section("Cérémonie", [
      row("Endroit", ceremony.location),
      row("Heure de début", formatTime(ceremony.start_time)),
      row("Heure de fin", formatTime(ceremony.end_time))
    ]),
    section("Entrée", [
    ...
    section("Cocktail", [
      row("Emplacement", cocktailLocationLabel(cocktail.cocktail_location)),
      row("Lieu / endroit", cocktail.cocktail_place),
      row("Distance DJ", cocktail.cocktail_dj_distance),
      row("Styles désirés", listLabel(cocktail.styles)),
      row("Demandes spéciales", cocktail.requests)
    ]),
    section("Repas", [
      row("Style(s) de musique", listLabel(dinner.styles)),
      row("Commentaires", dinner.requests)
    ]),
...
function buildPartyQuestionnaireSections(q) {
  ...
  return [
    section("Informations générales", [
      ...
      row("Tranche d'âge", g.age_range),
      row("Soirée thématique", ynLabel(g.thematic_evening)),
      row("Thème", g.thematic_evening === "no" ? null : listLabel(themes))
    ]),
    section("Déroulement", [
      row("Cocktail / accueil", ynLabel(sch.has_cocktail)),
      row("Repas / souper", ynLabel(sch.has_meal)),
      row("Type de souper", sch.meal_service_type),
      row("Bouchée", ynLabel(sch.meal_bouchee)),
    ...
    section("Musique cocktail", [
      row("Styles désirés", listLabel(cocktail.styles)),
      row("Demandes spéciales", cocktail.requests)
    ]),
    section("Musique souper", [
      row("Style(s) de musique", listLabel(dinner.styles)),
      row("Commentaires", dinner.requests)
    ]),
...
function buildTimelineSection(timelineItems) {
  ...
  const sorted = [...timelineItems].sort((a, b) => {
    ...
    return compareChronologicalTime(a.time, b.time);
  });
...
function formatTrailerRental(location, startDate, startTime, endDate, endTime) {
...
function formatSentLogList(logs) {
  if (!logs?.length) return "—";
  return logs
    .map((log) => formatDateTimeFr(log.sent_date, log.sent_time))
    .filter(Boolean)
    .join(" · ");
}

function formatContactLogList(logs) {
  if (!logs?.length) return "—";
  const { formatContactAt } = require("./client-contact-log");
  return logs
    .map((log) => formatContactAt(log.contact_date, log.contact_time, log.note))
    .filter(Boolean)
    .join(" · ");
}

function buildDjNotesSection(djNotes, emailRdvSentLogs = [], clientContactLogs = []) {
  const trailerRow =
  ...
  return section("Notes DJ (privées)", [
    ...
    row("Distance d'accès", djNotes?.tech_entry_distance),
    row("Nombre de portes", djNotes?.tech_entry_doors),
    row("Procédure pour entrer", djNotes?.tech_entry_procedure),
    row(
      "Plan de salle demandé",
      djNotes?.tech_floor_plan_requested === "yes" ? "Oui" : djNotes?.tech_floor_plan_requested === "no" ? "Non" : "—"
    ),
    row("Notes — plan de salle", djNotes?.tech_floor_plan_notes),
    trailerRow,
    roomRow,
    row("Commentaire client", djNotes?.tech_client_comment),
    row("Client contacté", formatContactLogList(clientContactLogs)),
    row("E-mail pour prendre RDV envoyé", formatSentLogList(emailRdvSentLogs)),
...
function buildSummarySheet({
  ...
  const sections = [
    ...
    buildDjNotesSection(djNotes, emailRdvSentLogs, clientContactLogs),
